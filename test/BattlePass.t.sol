...
contract BattlePassTest is Test {
    ...
    function setUp() public {
        bp = new BattlePass(creatorId, address(crafting), mockOwner);
        setUpRewards(bp);
    ...
    function setUpRewards(BattlePass _bp) public {
        RewardInfo memory _rewardInfo = RewardInfo(REWARD_TYPE.REDEEMABLE);
        rewardInfo.push(_rewardInfo);
        _rewardInfo = RewardInfo(REWARD_TYPE.REDEEMABLE);
        rewardInfo.push(_rewardInfo);
        _rewardInfo = RewardInfo(REWARD_TYPE.SPECIAL);
        rewardInfo.push(_rewardInfo);
        _bp.createReward(rewardInfo);

        // LootboxOption(rarity[2], ids[], qtys[])
        LootboxOption memory option = LootboxOption([0, 50], [101, 102], 1, 10);
        lootbox.push(option);
        option = LootboxOption([50, 100], [101, 102], [2, 20]);
        lootbox.push(option);
        return bp.newLootbox(lootbox);
    }

    function createLootbox(uint256 rewardOne, uint256 rewardTwo) public returns (uint256) {}

    function setUpSeason(BattlePass _bp) public {
        // LevelInfo(xp, freeId, premQty, premId, premQty, freeLim, premLim)
        LevelInfo memory _levelInfo = LevelInfo(1, 0, 0, 0, 0, 0, 0);
        levelInfo.push(_levelInfo);
        _levelInfo = LevelInfo(1, 1, 10, 2, 10);
    }
    ...
    // function testClaimRewardAtomicLootbox() public {
    //     levelInfo.pop();
    ...
    //     emit Delegated(mockUser, delegatee, delegateAmount);
    //     (bool success,) = address(bp).call(
    //         abi.encodePacked(abi.encodeWithSelector(bp.delegate.selector, delegatee, delegateAmount), mockUser)
    //     );
    //     assertTrue(success);
    //     assertEq(amount - delegateAmount, bp.balanceOf(mockUser, id));
    //     assertEq(bp.delegatedBy(mockUser, delegatee), delegateAmount);
