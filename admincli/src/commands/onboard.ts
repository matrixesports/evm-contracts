...
export default class Onboard extends Command {
  static description =
    "Onboarding: deploy CreatorToken and BattlePass, whitelists BattlePass and create season";
  helper = new Helper();
  ...
  public async run(): Promise<void> {
    ...
    this.log("deploying...");
    const pass = await this.deployer.deployBattlePass(
      creator_id,
      dbname,
      uri,
      this.crafting,
  ...
  async dirInit(creator_id: string) {
    let root = path.join(process.env.CREATORS_DIR!, creator_id);
